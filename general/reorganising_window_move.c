...
/* Put the window in some place depending on parameters */
XWindowChanges snap_to_position(const XWindowChanges current_pos,
                                ...
                                const double* position)
{
    ...
    switch(corner[0]) {
        ...
        case 'm':
            y_extra = current_pos.height / 2;
            break;

        case 'b':
            y_extra = current_pos.height;
            break;
    ...
    switch(corner[1]) {
        ...
        case 'm':
            x_extra = current_pos.width / 2;
            break;

        case 'r':
            x_extra = current_pos.width;
            break;

    }

    return_val.x = root_geom.x + (int)floor(root_geom.width * position[0]) - x_extra;
    return_val.y = root_geom.y + (int)floor(root_geom.height * position[1]) - y_extra;

    return return_val;
}

XWindowChanges snap_to_here(const XWindowChanges current_pos,
        const XWindowChanges root_geom,
        const char* position)
{
                        ...
                        const XWindowChanges root_geom)
{
    XWindowChanges return_val = current_pos;
    return_val.x =
        root_geom.x + (int)floor(root_geom.width * LEFT_EDGE);
    return_val.y =
        root_geom.y + (int)floor(root_geom.height * TOP_EDGE);
    return return_val;
}
                           ...
                           const XWindowChanges root_geom)
{
    XWindowChanges return_val = current_pos;
    int middle_vertical =
        root_geom.y + (root_geom.height) / 2;
    return_val.x =
        root_geom.x + (int)floor(root_geom.width * LEFT_EDGE);
...
XWindowChanges resize_to_this(const XWindowChanges cur_geom,
        const XWindowChanges root_geom,
...
int read_workarea_direct(Display* dpy, Window root_win, XWindowChanges* root_geom)
{
    int retval = 1;
    Atom* return_values;
...
int main(int argc, char *argv[])
{
     ...
     *  window.
     */
    new_geom = get_new_size(focussed_geom, root_geom, argv[2]);
