# -*- encoding: utf-8 -*-
#
# NOTE: The default file is in /etc/xdg/subtle/subtle.rb
#
...
screen 1 do
  # top    [ :views, :title, :spacer, :keychain, :spacer, :tray, :sublets, :clock ]
  top    [ :views, :separator, :wifi, :title, :spacer, :tasks, :spacer, :mpd, :separator, :volume, :separator, :clock ]
  bottom [ ]
end
...
# My additions
gravity :ranger_low,     [   0,  60, 100,  40 ]
...
# Should change this - I don't have a keypad
#grab "KP_Add",      :ViewNext
#grab "KP_Subtract", :ViewPrev
grab "W-C-Up",      :ViewNext
grab "W-C-Down", :ViewPrev
...
grab "W-q", [ :top_left,     :top_left33,     :top_left66     ]
grab "W-w", [ :top,          :top33,          :top66          ]
grab "W-e", [ :top_right,    :top_right33,    :top_right66    ]
grab "W-a", [ :left,         :left33,         :left66         ]
grab "W-s", [ :center,       :center33,       :center66       ]
...
#
# QWERTY
grab "W-z", [ :bottom_left,  :bottom_left33,  :bottom_left66  ]
#
grab "W-x", [ :bottom,       :bottom33,       :bottom66       ]
grab "W-c", [ :bottom_right, :bottom_right33, :bottom_right66 ]
...
tag "testterm" do
  match :instance => "testterm"
  gravity :testoutterm
  # This geometry is not good - need to change from percentage of screen
  # to pixels -- just use the gravity for now
  # geometry [   60, 64,  38,  34 ]
end

tag "terms" do
  match :name =>"xterm|ranger"
end
...
# Upon start, create a tag for each view
# on :start do
